...
# ultra compression (level 9), then split into ≤45 MB parts so every committed
...
#
# Target part sizes after 7-zip level-9 gzip + split at 45 MB:
...
CHECKSUMS="$TOOLCHAIN_DIR/checksums.sha256"
{
    ...
    printf '# Compressed with 7-zip ultra (level 9), split into ≤45 MB parts\n'
...
echo "Running 'bash scripts/install.sh' on a no-Rust machine will reassemble"
