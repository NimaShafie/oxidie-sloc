...
echo ""
echo "  3. On each air-gapped Windows endpoint:"
echo "       # No admin needed (current user) — this is what install.sh does automatically:"
echo "       certutil -user -addstore Root deploy\certs\sloc-ca.crt"
echo "       # Or machine-wide (requires Admin):"
echo "       Import-Certificate -FilePath deploy\certs\sloc-ca.crt -CertStoreLocation Cert:\LocalMachine\Root"
echo ""
