    ...
    stages {
        ...
        stage('Vendor sources') {
            steps {
                sh '''
                    if [ ! -d vendor ]; then
                        echo "Verifying vendor.tar.xz integrity (FIND-017)..."
        ...
        stage('Analyze') {
            steps {
                script {
                    ...
                    // FIND-018 / SECURITY NOTE: This relaxes Jenkins' global CSP to allow
                    // inline styles and scripts so that the oxide-sloc HTML report renders
                    // correctly in the Jenkins artifact browser.
                    //
                    // Risk accepted because:
                    //   1. The report is a static artifact generated by the CI job itself —
                    //      it contains no user-supplied HTML, only output from oxide-sloc.
                    //   2. The CSP change is scoped to the Jenkins artifact viewer, not the
                    //      wider Jenkins UI.
                    //   3. Admin script approval is required; unapproved it silently no-ops.
                    //
                    // Alternative (preferred for high-assurance environments): serve the
                    // HTML report from a separate origin (e.g. GitHub Pages, S3) where you
                    // control the CSP response header without touching Jenkins' global policy.
                    try {
                        System.setProperty(
                    ...
                    // Validate free-text parameters before use in shell commands (FIND-004).
                    // MIXED_LINE_POLICY and CI_PRESET are choice parameters but validated here
                    // as defence-in-depth. SCAN_PATH and REPORT_TITLE are passed via withEnv
                    // (shell variables, not Groovy string interpolation) to prevent injection.
                    // Allowlist validation for choice and free-text parameters (FIND-004).
                    ...
                    // OUTPUT_SUBDIR: only safe path characters allowed (no ;, &, |, $, etc.)
                    ...
                    // Glob patterns: allow common glob characters only.
                    ...
                    if (params.ENABLED_LANGUAGES) {
                        params.ENABLED_LANGUAGES.tokenize(',').each { l ->
                            if (!(l.trim() ==~ /^[a-zA-Z0-9\+\#]+$/)) { error("ENABLED_LANGUAGES contains invalid value: ${l.trim()}") }
                        }
                    }
                    ...
                    // Glob/language args: each validated token passed as individual --flag value.
                    // Validated above; concatenated here after allowlist check.
                    ...
                    // a. Quick plain summary — SCAN_PATH via withEnv prevents injection.
                    ...
                    // b. Main artifact run — all free-text params via withEnv shell vars.
                    withEnv([
                    ...
                    // e. HTML content sanity checks — REPORT_TITLE via withEnv to prevent injection.
        ...
        stage('Policy variants') {
            steps {
                script {
                    // FIND-004: SCAN_PATH passed via withEnv; policy comes from a Groovy literal list.
