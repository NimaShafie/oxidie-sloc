...
// ── Post-hoc author identity merging ─────────────────────────────────────────
//
// The attribution pass auto-merges only the high-confidence same-email case. Real repos also
// accumulate the *same person under different emails* (corporate login, personal address, a
// bare "First Last <first@laptop>"). `IdentityMap` lets an operator combine those identities
// after a scan — without re-running blame — by grouping their emails under one canonical name.

/// One operator-defined merge: several email identities that are actually one person.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct AuthorMergeGroup {
    /// Display name for the merged contributor.
    pub canonical_name: String,
    /// Primary email for the merged contributor (also used as the group's stable key).
    pub canonical_email: String,
    /// Every email folded into this identity (lower-cased), including `canonical_email`.
    pub members: Vec<String>,
}

/// A persisted collection of [`AuthorMergeGroup`]s. Applied to a run's authors at display time.
...
impl IdentityMap {
    ...
    pub fn group_for(&self, email: &str) -> Option<&AuthorMergeGroup> {
        ...
        self.groups.iter().find(|g| g.members.contains(&key))
    }
    ...
    pub fn merge(&mut self, emails: &[String], name: Option<&str>) {
        let mut members: Vec<String> = emails
            .iter()
        ...
        // Absorb any existing groups that overlap the selection.
        let mut absorbed: Vec<String> = Vec::new();
        self.groups.retain(|g| {
            if g.members.iter().any(|m| members.contains(m)) {
                absorbed.extend(g.members.iter().cloned());
                false
            } else {
                true
            }
        ...
        members.sort();
        members.dedup();
        let canonical_email = members[0].clone();
        let canonical_name = name
            .map(str::trim)
            .filter(|s| !s.is_empty())
            .map_or_else(|| canonical_email.clone(), ToString::to_string);
        self.groups.push(AuthorMergeGroup {
            canonical_name,
            canonical_email,
            members,
        });
    ...
    pub fn unmerge(&mut self, canonical_email: &str) {
        let key = canonical_email.trim().to_lowercase();
        self.groups
            .retain(|g| g.canonical_email.to_lowercase() != key);
    }
    ...
    pub fn to_mailmap(&self) -> String {
        let mut out = String::from(
            "# Generated by oxide-sloc — maps alternate author emails to a canonical identity.\n",
        );
        for g in &self.groups {
            for member in &g.members {
                if *member == g.canonical_email.to_lowercase() {
                    continue;
                }
                out.push_str(&format!(
                    "{} <{}> <{}>\n",
                    g.canonical_name, g.canonical_email, member
                ));
            }
        }
        out
...
pub fn apply_identity_map(run: &mut AnalysisRun, map: &IdentityMap) {
    ...
    authors
        ...
        .map(|a| {
            map.group_for(&a.canonical_email).map_or_else(
                ...
                |g| {
                    (
                        g.canonical_email.to_lowercase(),
                        g.canonical_name.clone(),
                        g.canonical_email.clone(),
                    )
                },
            )
        })
...
fn merge_authors(
    ...
    for (old_idx, (key, name, email)) in resolved.iter().enumerate() {
        ...
        for alias in &src.aliases {
            if !dst.aliases.contains(alias) {
                dst.aliases.push(alias.clone());
            }
        }
    }
    (merged, old_to_new)
    ...
    for rec in records {
        if let Some(ownership) = rec.ownership.as_mut() {
            ...
            for entry in ownership.iter() {
                let new_id = old_to_new[entry.author_id as usize];
                by_new.entry(new_id).or_default().add(&entry.counts);
            }
            let mut folded: Vec<FileOwnership> = by_new
                .into_iter()
                .map(|(author_id, counts)| FileOwnership { author_id, counts })
                .collect();
            folded.sort_by_key(|e| std::cmp::Reverse(e.counts.total_lines));
            *ownership = folded;
        }
    }
}
...
fn sort_and_reindex_authors(run: &mut AnalysisRun, merged: Vec<Author>) {
    ...
    order.sort_by(|&a, &b| {
        merged[b]
            .counts
            .code_lines
            .cmp(&merged[a].counts.code_lines)
            .then_with(|| merged[a].canonical_name.cmp(&merged[b].canonical_name))
    });
    let mut remap = vec![0u32; merged.len()];
    for (new_id, &old) in order.iter().enumerate() {
        remap[old] = new_id as u32;
    }
    ...
    let mut sorted: Vec<Author> = order.iter().map(|&old| merged[old].clone()).collect();
    for (new_id, author) in sorted.iter_mut().enumerate() {
        author.id = new_id as u32;
    }
    run.authors = sorted;
...
";
    ...
    // ── Post-hoc identity merging ─────────────────────────────────────────────

    fn author(id: u32, name: &str, email: &str, code: u64) -> Author {
        Author {
            id,
            canonical_name: name.into(),
            canonical_email: email.into(),
            aliases: vec![RawIdentity {
                name: name.into(),
                email: email.into(),
            }],
    ...
    /// A minimal `AnalysisRun` carrying the given authors and one empty-ownership file record.
    fn minimal_run_with_authors(authors: Vec<Author>) -> AnalysisRun {
        AnalysisRun {
            tool: ToolMetadata {
                name: "sloc".into(),
                version: "0.0.1".into(),
                run_id: "merge-test".into(),
    ...
    #[test]
    fn identity_map_merge_and_unmerge() {
        let mut map = IdentityMap::default();
        map.merge(
            &["nima@corp.com".into(), "nima@personal.com".into()],
            Some("Nima Shafie"),
        );
        assert_eq!(map.groups.len(), 1);
        assert!(map.group_for("NIMA@CORP.COM").is_some(), "case-insensitive");
        // Merging an overlapping selection extends the same group rather than duplicating.
        map.merge(
            &["nima@corp.com".into(), "nima@laptop.com".into()],
            Some("Nima Shafie"),
        );
        assert_eq!(map.groups.len(), 1);
        assert_eq!(map.groups[0].members.len(), 3);
        let canonical = map.groups[0].canonical_email.clone();
        map.unmerge(&canonical);
        assert!(map.groups.is_empty());
    }

    #[test]
    fn identity_map_to_mailmap_lists_aliases() {
        let mut map = IdentityMap::default();
        map.merge(&["a@x.com".into(), "b@y.com".into()], Some("Real Name"));
        let mm = map.to_mailmap();
        // Canonical (a@x.com, the sorted-first) is not emitted as its own alias line.
        assert!(mm.contains("Real Name <a@x.com> <b@y.com>"));
        assert_eq!(mm.matches("Real Name <").count(), 1);
    }

    #[test]
    fn apply_identity_map_folds_authors_and_ownership() {
        let mut run = minimal_run_with_authors(vec![
            author(0, "Nima Shafie", "nima@corp.com", 100),
            author(1, "nshafie", "nima@personal.com", 40),
            author(2, "Other", "other@x.com", 30),
        ]);
        // One file owned across the two soon-to-merge identities.
        ...
        apply_identity_map(&mut run, &map);

        assert_eq!(run.authors.len(), 2, "two identities folded into one");
        let nima = run
            .authors
            .iter()
            .find(|a| a.canonical_name == "Nima Shafie")
            .expect("merged author present");
        assert_eq!(nima.counts.code_lines, 140, "counts summed");
        assert_eq!(nima.aliases.len(), 2, "both aliases retained");
        assert_eq!(run.authors[0].canonical_name, "Nima Shafie", "sorts first");
